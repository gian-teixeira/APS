...
from view.provider.StudentConsumptionProvider import StudentConsumptionProvider

import tkinter as tk
from tkinter import ttk

class StudentConsumptionSearch(ttk.Frame):
    def __init__(self):
        super().__init__()

        self.controller = StudentConsumptionController()
        self.card = None
        self.selected_name = None
        self.right = ttk.Frame(self)
        self.left = ttk.Frame(self)
        self.search = SearchBox(
            self.controller, StudentConsumptionProvider,
            ["Id"], tk.SINGLE, self.selection_callback())
    ...
    def selection_callback(self):
        def callback(event):
            ...
            card = Card(StudentConsumptionProvider, self.selected, self.delete)
